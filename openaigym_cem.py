...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', dest='n_iter', help="number of episodes",
            type=int, default=10)
    ...
    env.seed(0)
    np.random.seed(0)
    params = dict(n_iter=args.n_iter, batch_size=25, elite_frac = 0.2)
    num_steps = 200
    ...
    def noisy_evaluation(theta):
        ...
        agent = ContinuousActionLinearPolicy(theta, 7, 7)
    ...
    for (i, iterdata) in enumerate(
        ...
        #cem(noisy_evaluation, 0.0*np.random.random_sample(env.observation_space.shape[0]+1), **params)):
        print('Iteration %2i. Episode mean reward: %7.3f'%(i, iterdata['y_mean']))
        ys.append(iterdata['y_mean'])
    ...
    if args.plot:
        fig, ax = plt.subplots()
        plt.plot(range(1,len(ys)+1), ys, lw=2)
        plt.title(args.target)
        plt.xlabel('episode')
        plt.ylabel('reward')
        plt.savefig(args.output + '.png', transparent=True)
